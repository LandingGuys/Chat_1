...
<link type="text/css" rel="stylesheet" href="css/main.css" />
  ...
  <div class="row div_function">
  	...
  	<div class="container div_divider">
  		...
  		<div class=" pull-right" style="padding:20px">
 					共有${requestScope.pageModel.totalPage}页  当前是${requestScope.pageModel.currentPage }  页<br/>
  					<a href="<%=request.getContextPath() %>/ArticleServlet?method=total&user_id=${user.user_id}&currentPage=1">首页</a>
  					<a href="<%=request.getContextPath()%>/ArticleServlet?method=total&user_id=${user.user_id}&currentPage=${requestScope.pageModel.prePage}">上一页</a>
  					<a href="<%=request.getContextPath()%>/ArticleServlet?method=total&user_id=${user.user_id}&currentPage=${requestScope.pageModel.nextPage}">下一页</a>
  					<a href="<%=request.getContextPath()%>/ArticleServlet?method=total&user_id=${user.user_id}&currentPage=${requestScope.pageModel.totalPage}">尾页</a>     
  	...
  	<script>
  function searchBook(){
	  window.location.href="<%=request.getContextPath()%>/ArticleServlet?method=total&user_id=${user.user_id}&currentPage="+$("#pageNo").val();
  }
</script>
  </body>
  </html>